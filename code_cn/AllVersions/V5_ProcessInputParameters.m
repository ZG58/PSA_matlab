function InputParams = V5_ProcessInputParameters(process_vars, material, N)
...
%% 将值分配给必要的变量
    ...
    Params(33) = y_0          ;   % 逆流降压出口y的位置 = 重组分回流入品y: y_HP
                                  % y_HR1 = y_0 - 重组分回流步骤中入口CO2摩尔分数的初始猜测值
    Params(34) = T_0          ;   % 逆流降压出口T的位置 = 重组分回流入品T: T_HP
                                  % T_HR1 = T_0 - 重组分回流步骤中入口温度的初始猜测值
    Params(35) = ndot_0*beta  ;   % 300/30   % 逆流降压出口ndot的位置 = 重组分回流入品ndot
                                  % ndot_HR1 = ndot_0*beta - 重组分回流步骤中入口ndot的初始猜测值
    ...
    Params(39) = y_0          ;   % 逆流降压出口y的位置 = 重组分回流入品y: y_HP
                                  % y_HR2 = y_0 - 重组分回流步骤中入口CO2摩尔分数的初始猜测值
    Params(40) = T_0          ;   % 逆流降压出口T的位置 = 重组分回流入品T: T_HP
                                  % T_HR2 = T_0 - 重组分回流步骤中入口温度的初始猜测值
    Params(41) = ndot_0*beta  ;   % 300/30   % 逆流降压出口ndot的位置 = 重组分回流入品ndot
                                  % ndot_HR2 = ndot_0*beta - 重组分回流步骤中入口ndot的初始猜测值
...
%% 经济参数
    ...
    % 根据要模拟的循环更改此项
    cycle_time = t_pres + t_ads + t_HR1 + t_CoCdepres + t_HR2 + t_CnCdepres + t_LR ;   % 1个循环所需的总时间 [s]
